...
# 列表的索引
print(items3[0], items3[-size])        # 35 35  **负数索引从列表的末尾开始计数，-1 是最后一个元素**
items3[-1] = 100
print(items3[size - 1], items3[-1])    # 100 100

# 列表的切片                            **items[start:end:step],start是起始索引（包含），end是结束索引（不包含），step是提取间隔**
